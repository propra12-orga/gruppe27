...
public class Spielfeld extends JPanel implements ActionListener, Serializable {
	...
	/**
	 * L‰dt aus LevelGen ein generiertes Level-Objekt. Sollte das Level
	 * konsistent sein wird es zur¸ckgegeben. Wenn nicht wird ein neues Level
	...
	private static Level loadlevel(int levelnr) {
		...
		/* Multiplayer Zufalls-Map pr¸fen */
		if ((system.getamplayer() > 1) || (system.getboolLAN())) {
			...
			/*
			 * Wir pr¸fen alle drei Player-Startpunkte, ob sie erreichbar sind.
			 */
			if ((PathFinder
			...
			LevelGen newLevel = new LevelGen(system.getfeldx(),
					system.getfeldy(), system.getamplayer());
			/*
			 * Nur pr¸fen, ob Exit erreichbar ist, da Solo-Game.
			 */
			...
			if ((PathFinder.check(newLevelArray, 1, 1,
				...
				return null; /*
							 * Man ist gezwungen etwas zur¸ckzugeben. Verursacht
	...
	protected void paintComponent(Graphics g) {
		...
		for (int i = 0; i < level.getlaenge(); i++)
			for (int j = 0; j < level.getbreite(); j++) {
				...
				// TODO Checken wieso die n√§chsten 2 Zeilen n√∂tig sind
	...
	// nicht l√∂schen, braucht man f√ºr Netzwerk
